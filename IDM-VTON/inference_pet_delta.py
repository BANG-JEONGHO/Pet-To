...
def parse_args():
    ...
    parser.add_argument("--finetuned_delta_path", type=str, default=None, required=False, help="Path to checkpoint-final-* folder containing unet_trainable_delta/trainable_delta.safetensors.")
...
def main():
    ...
    if args.finetuned_delta_path is not None:
        from safetensors.torch import load_file

        delta_file = os.path.join(
            args.finetuned_delta_path,
            "unet_trainable_delta",
            "trainable_delta.safetensors",
        )

        print(f"[Pet-VTON inference] loading UNet trainable delta from: {delta_file}")
        delta_state = load_file(delta_file, device="cpu")
        missing, unexpected = unet.load_state_dict(delta_state, strict=False)

        print(f"[Pet-VTON inference] loaded delta tensors: {len(delta_state)}")
        print(f"[Pet-VTON inference] missing keys: {len(missing)}")
        print(f"[Pet-VTON inference] unexpected keys: {len(unexpected)}")

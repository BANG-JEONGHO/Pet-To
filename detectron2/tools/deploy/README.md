...
Quick local run (auto-build + runtime library path setup):
```
./run_torchscript_mask_rcnn.sh output/model.ts input.jpg tracing
```

End-to-end local run (export + C++ inference in one command):
```
./run_end_to_end.sh input.jpg tracing
```
